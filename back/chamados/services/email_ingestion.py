...
def processar_emails():
    ...
    try:
        ...
        logger.error(f"Erro de autenticacao/conexao IMAP: {e}")
        erros += 1
    except Exception as e:
        logger.error(f"Erro inesperado na ingestao de email: {e}", exc_info=True)
        erros += 1

    logger.info(f"Ingestao concluida: {processados} chamados criados, {erros} erros")
    return {"enabled": True, "processados": processados, "erros": erros}
