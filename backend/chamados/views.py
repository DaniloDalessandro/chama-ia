...
from .models import Chamado, AnexoChamado, ComentarioChamado, HistoricoChamado, Notification, WebhookConfig, EmailIngestionConfig
from .serializers import (
    ...
    EmailIngestionConfigSerializer,
)
...
# ============================================
# View de Configuracao de Email (ingestao de chamados)
# ============================================

class EmailIngestionConfigView(APIView):
    """
    Configuracao da caixa de email usada para criar chamados automaticamente.
    Apenas admins podem visualizar/editar. Modelo singleton (uma unica linha).

    Endpoints:
    - GET /api/v1/chamados/email-config/ - Ver configuracao atual
    - PUT /api/v1/chamados/email-config/ - Criar/atualizar configuracao
    """
    ...
    def get(self, request):
        if not request.user.is_staff:
            return Response({"detail": "Apenas administradores podem acessar esta configuracao."}, status=status.HTTP_403_FORBIDDEN)

        instance = EmailIngestionConfig.objects.first()
        if not instance:
            return Response({"configured": False})

        data = EmailIngestionConfigSerializer(instance).data
        data["configured"] = True
        return Response(data)

    def put(self, request):
        if not request.user.is_staff:
            return Response({"detail": "Apenas administradores podem editar esta configuracao."}, status=status.HTTP_403_FORBIDDEN)

        instance = EmailIngestionConfig.objects.first()

        if not instance and not request.data.get("password"):
            return Response({"password": ["Senha e obrigatoria ao cadastrar o email pela primeira vez."]}, status=status.HTTP_400_BAD_REQUEST)

        serializer = EmailIngestionConfigSerializer(instance, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
...
class EmailIngestionCheckNowView(APIView):
    """
    Dispara a verificacao da caixa de email imediatamente (sem esperar o
    agendamento periodico do Celery Beat).

    Endpoint: POST /api/v1/chamados/email-config/verificar-agora/
    """
    ...
    def post(self, request):
        if not request.user.is_staff:
            return Response({"detail": "Apenas administradores podem executar esta acao."}, status=status.HTTP_403_FORBIDDEN)

        if not EmailIngestionConfig.objects.exists():
            return Response({"detail": "Nenhum email configurado ainda."}, status=status.HTTP_400_BAD_REQUEST)
