...
class EmailIngestionConfig(models.Model):
    """
    Configuracao da caixa de email (IMAP) usada para criar chamados
    automaticamente a partir de emails recebidos. Modelo singleton:
    ...
    def set_password(self, raw_password):
        """Criptografa e armazena a senha do email."""
        if not settings.WEBHOOK_ENCRYPTION_KEY:
            raise ValueError("WEBHOOK_ENCRYPTION_KEY not configured in settings")
    ...
    def get_password(self):
        """Descriptografa e retorna a senha do email."""
        if not settings.WEBHOOK_ENCRYPTION_KEY:
            raise ValueError("WEBHOOK_ENCRYPTION_KEY not configured in settings")
    ...
    def registrar_verificacao(self, processados, erro=""):
        """Atualiza estatisticas apos uma verificacao de emails."""
        from django.utils import timezone
        self.last_checked_at = timezone.now()
        self.last_error = erro
        self.total_processed += processados
        self.save(update_fields=["last_checked_at", "last_error", "total_processed"])
